...
import { CoursesType } from '../../../../../models';
...
export const AddTipoCursos: React.FC<Props> = (props) => {
    ...
    const hadleGetId = async () => {
        try {
            const response = await api.get<CoursesType[]>('Configuration/GetIdCoursesType', { id: props.idTipoCurso });
    ...
    const handleRegistrar = async (values: FormikValues) => {
        ...
        try {
            // Solicitud POST
            if (values.id > 0) {
                await api.put<any>('Configuration/PutCoursesType', values);
            } else {
                await api.post<any>('Configuration/PostCoursesType', values);
            }
    ...
    return (
        <>
                    ...
                    <div className="AddCursos_Formulario">
                        <Formik
                            ...
                            validate={(valor) => {
                                ...
                                if (!valor.codigo) {
                                    errors.codigo = 'Introduce un codigo para el tipo de documento';
                                }
            ...
            >
                ...
                >
                    {props.idTipoCurso == 0 ? (
                        "¡Tipo de curso guardado exitosamente!"
                    ) : (
                        "¡Tipo de curso actualizado exitosamente!"
                    )}
