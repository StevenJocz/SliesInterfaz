...
import { CoursesCategories } from "../../../../../models";
...
const AddCategorias: React.FC<Props> = (props) => {
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [msg, setMsg] = useState('');
    const submitButtonRef = useRef<HTMLButtonElement>(null);
    const [data, setData] = useState<CoursesCategories>();
    ...
    const hadleGetId = async () => {
        try {
            const response = await api.get<CoursesCategories[]>('Configuration/GetIdCoursesCategories', { id: props.idCategoria });
    ...
    const handleRegistrar = async (values: FormikValues) => {
        ...
        try {
            // Solicitud POST
            if (values.id > 0) {
                await api.put<any>('Configuration/PutCategories', values);
            } else {
                await api.post<any>('Configuration/PostCoursesCategories', values);
            }
                    ...
                    <div className="AddCursos_Formulario">
                        <Formik
                            ...
                            validate={(valor) => {
                                ...
                                if (!valor.codigo) {
                                    errors.codigo = 'Introduce un código para el tipo de documento';
                                }
                                    ...
                                    <div className='Configuracion_Formuario'>
                                        ...
                                        <div className='Configuracion_Formuario-right'>
                                            <StyledTextField
                                                ...
                                                size="small"
                                                color="secondary"
                                                placeholder='Introduce el codigo categorias'
                                                value={values.codigo}
                                                onChange={(e) => setFieldValue('codigo', e.target.value)}
                                            />
                                            <ErrorMessage name='nombre' component={() => <p className='Error'>{errors.codigo}</p>} />
                                        </div>
                                        <div className='Configuracion_Formuario-right'>
                                            <StyledTextField
                                                name='titulo'
                                                ...
                                                size="small"
                                                color="secondary"
                                                placeholder='Introduce el nombre categorias'
                                                value={values.nombre}
